...
export async function getExternalNews(category = null, count = 10) {
  ...
  try {
    ...
    let url = new URL(`${BASE_URL}/top-headlines`);
    let params = {
      apiKey: API_KEY,
      pageSize: count,
      language: 'en'
    ...
    if (category && category !== 'all') {
      params.category = mapCategoryToAPI(category);
    }
    ...
    const articles = data.articles.map(article => ({
      ...
      excerpt: truncateText(article.description || article.title || '', 150),
...
export async function getExternalBreakingNews(count = 5) {
  ...
  try {
    ...
    const breakingNews = data.articles.map(article => ({
      ...
      content: article.content || article.description || 'Breaking news update',
      excerpt: truncateText(article.description || article.title || '', 100),
      image: article.urlToImage || 'img/placeholder.jpg',
      publishDate: new Date(article.publishedAt || Date.now()),
...
export async function searchNews(query, count = 20) {
  ...
  try {
    let url = new URL(`${BASE_URL}/everything`);
    let params = {
      apiKey: API_KEY,
      q: query.trim(),
      pageSize: count,
    ...
    const searchResults = data.articles.map(article => ({
      ...
      url: article.url || '#',
      category: 'news',
      subcategory: determineCategory(article),
      breaking: false
    }));
    ...
    return []; // Empty array for search errors
  }
}

/**
 * Determine the best category for an article based on its content and source
...
function determineCategory(article) {
  ...
  if (content.includes('politic') || content.includes('election') || content.includes('government')) {
    return 'politics';
  }
  if (content.includes('business') || content.includes('economy') || content.includes('stock') || content.includes('market')) {
    return 'business';
  }
  if (content.includes('tech') || content.includes('software') || content.includes('app') || content.includes('digital')) {
    return 'technology';
  }
  if (content.includes('health') || content.includes('medical') || content.includes('doctor') || content.includes('patient')) {
    return 'health';
  }
  if (content.includes('sport') || content.includes('game') || content.includes('team') || content.includes('player')) {
    return 'sports';
  }
  if (content.includes('entertainment') || content.includes('movie') || content.includes('celebrity') || content.includes('actor')) {
...
function mapAPIToCategory(source, defaultCategory) {
  if (!source) return defaultCategory;
...
function getFallbackNewsData(category = null, count = 10) {
  ...
  if (category && category !== 'all') {
    ...
    // If filtering resulted in no articles, return general articles
